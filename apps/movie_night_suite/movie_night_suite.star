...
def main(config):
    ...
    print("Searching for movie: '{}'".format(movie_name))
...
def marquee_widget(data, simple=False):
    ...
    def build_frame(offset):
        row_dots = render.Row(children=[render.Box(width=1, height=1, color="#fff" if (i % 2) == offset else "#000") for i in range(64)])
...
def details_widget(data):
    ...
    return render.Column(
        main_align="start", expanded=True,
        children=[
            render.Box(height=1),
            render.Row(
...
def poster_widget(data):
    ...
    poster_res = http.get(poster_url)
    ...
    return render.Box(
        ...
        child=render.Marquee(
            width=64, height=32, scroll_direction="vertical", delay=30,
...
def ratings_widget(data):
    ...
    for r in ratings:
        s = r.get("Source")
        v = r.get("Value")
        if s == "Internet Movie Database": imdb = v
        elif s == "Rotten Tomatoes": rt = v
        elif s == "Metacritic": meta = v.split("/")[0]

    def parse_score(val):
        if val == "N/A": return 0
        if "%" in val: return int(val.replace("%", ""))
        if "/" in val: return float(val.split("/")[0]) / float(val.split("/")[1]) * 100
    ...
    return render.Padding(
        pad=(2, 0, 2, 0),
        child=render.Column(
            main_align="space_evenly", expanded=True,
            children=[
                # IMDb
                render.Row(
                    main_align="space_between", expanded=True, cross_align="center",
                    children=[
